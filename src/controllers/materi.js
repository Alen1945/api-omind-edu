...
exports.CreateMateri = async (req, res, next) => {
  ...
  if (req.file) {
    ...
    pathImage = `https://firebasestorage.googleapis.com/v0/b/${bucket.name
    ...
    if (materi) {
      res.status(201).json({
        ...
        data: {
          ...
          description: materi.get('ingredients'),
  ...
  try {
    ...
    if (materi) {
      ...
      if (updateData && pathImage && materi.dataValues.image) {
        await bucket
          .file(
            `materis-image/${materi.dataValues.image
              .substring(
...
exports.DeleteMateri = async (req, res, next) => {
  try {
    ...
    if (dataMateri.dataValues.image) {
      await bucket
        .file(
          `materi-image/${dataMateri.dataValues.image
            .substring(
